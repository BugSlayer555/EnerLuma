...
const statusConfig: Record<string, { color: string; bg: string; label: string; icon: typeof Wifi }> = {
    online: { color: '#10b981', bg: '#10b98112', label: 'Online', icon: Wifi },
    offline: { color: '#ef4444', bg: '#ef444412', label: 'Offline', icon: WifiOff },
    degraded: { color: '#f59e0b', bg: '#f59e0b12', label: 'Degraded', icon: AlertTriangle },
    maintenance: { color: '#6366f1', bg: '#6366f112', label: 'Maintenance', icon: Clock },
}
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
                ...
                <h2 className="text-lg font-bold text-white">Device Health Monitor</h2>
                ...
                {devices.map((d, i) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className="rounded-xl p-4 group hover:scale-[1.01] transition-transform"
                            style={{
                                background: d.status === 'offline' ? 'rgba(239,68,68,0.04)' : 'rgba(255,255,255,0.03)',
                                border: `1px solid ${d.status === 'offline' ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.07)'}`,
                            }}
                        >
                            {/* Header */}
                            ...
                            <div className="flex items-center gap-2 mb-3 text-[10px]">
                                ...
                                <span className="text-gray-400">Uptime: <span className="text-white font-semibold">{d.uptime}</span></span>
                            </div>

                            {/* Metrics */}
                            <div className="grid grid-cols-3 gap-2 mb-3">
                                {d.metrics.map(m => (
                                    <div key={m.label} className="p-2 rounded-lg text-center" style={{ background: `${metricStatusColor[m.status]}08` }}>
                                ...
                                <div className="space-y-1">
                                    {d.issues.map((issue, j) => (
                                        <div key={j} className="flex items-center gap-1.5">
                                            ...
                                            <span className="text-[10px] text-gray-400">{issue}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            {d.issues.length === 0 && (
                                <p className="text-[10px] text-emerald-400/70">✓ No issues detected</p>
                            )}
                        </motion.div>
                    )
                })}
