...
import UsageAlertCard from '../components/UsageAlertCard'
...
const alerts = [
  { id: 1, type: 'warning', severity: 'high', message: 'HVAC consuming 40% above average', time: '5 min ago', icon: AlertTriangle },
  { id: 2, type: 'info', severity: 'medium', message: 'Smart Thermostat firmware v2.3 available', time: '1 hour ago', icon: Info },
  { id: 3, type: 'success', severity: 'low', message: 'Water leak test passed — all sensors normal', time: '3 hours ago', icon: CheckCircle },
]

const recommendations = [
  { id: 1, title: 'Switch HVAC to Eco Mode', description: 'Current HVAC usage is 40% above your weekly average. Switching to eco mode during 10 AM – 4 PM can save energy.', impact: 'Save ~₹340/month', priority: 'high', icon: Zap },
  { id: 2, title: 'Optimize Water Heater Schedule', description: 'Your water heater runs 6 hours daily. Reducing to 4 hours with a smart schedule covers your usage.', impact: 'Save ~₹180/month', priority: 'medium', icon: Droplets },
  { id: 3, title: 'Upgrade Smart Thermostat', description: 'Your thermostat is 5 years old. Upgrading to an AI-enabled model can reduce energy costs by 10-15%.', impact: 'Save ~₹240/month', priority: 'low', icon: TrendingUp },
]

const smartOverview = [
  { label: 'Energy Status', value: 'Optimal', detail: '2.4 kWh today, 12% lower than yesterday', tone: 'text-emerald-600 bg-emerald-50' },
  { label: 'Water Status', value: 'Stable', detail: '42 L today with no leak alerts', tone: 'text-cyan-600 bg-cyan-50' },
  { label: 'Alerts', value: '1 High', detail: 'HVAC usage spike needs attention', tone: 'text-amber-600 bg-amber-50' },
  { label: 'Efficiency Score', value: '87/100', detail: 'Strong performance this week', tone: 'text-primary-700 bg-primary-50' },
]

const analyticsHighlights = {
  energy: [
    { title: 'Usage Trend', value: 'Down 8%', note: 'Lower than your 30-day average' },
    { title: 'Pattern Insight', value: 'Peak 6-8 PM', note: 'HVAC + lighting drives evening peaks' },
    { title: 'AI Efficiency Tip', value: 'Eco mode suggested', note: 'Potential savings ~₹340/month' },
  ],
  water: [
    { title: 'Usage Trend', value: 'Up 3%', note: 'Slight increase vs last week' },
    { title: 'Anomaly Check', value: 'No leaks', note: 'Flow pattern appears normal' },
    { title: 'Conservation Tip', value: 'Heater schedule', note: 'Reduce runtime from 6h to 4h/day' },
  ],
}
...
export default function Dashboard() {
  ...
  /* Mock threshold data — in production this comes from API */
  const weeklyEnergyUsage = weeklyData.reduce((sum, d) => sum + d.energy, 0)
  const weeklyWaterUsage = weeklyData.reduce((sum, d) => sum + d.water, 0)
  const energyThreshold = 120 // kWh per week
  const waterThreshold = 900 // Liters per week
  const isOverThreshold = weeklyEnergyUsage > energyThreshold || weeklyWaterUsage > waterThreshold

  return (
    <>
      {/* Usage Alert Overlay */}
      {showUsageAlert && isOverThreshold && (
        <UsageAlertCard
          alert={{
            type: 'both',
            energyUsage: Math.round(weeklyEnergyUsage * 10) / 10,
            energyThreshold,
            waterUsage: Math.round(weeklyWaterUsage),
            waterThreshold,
          }}
      ...
      >
        ...
        <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Welcome back, {userName}. Here's your home overview.</p>
          </div>
        ...
        <motion.div variants={fadeUp} className="card-static">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="section-title">Smart Dashboard Overview</h2>
              <p className="text-xs text-gray-400 mt-0.5">Quick snapshot of energy, water, alerts, and efficiency status</p>
            </div>
            <button
              type="button"
            ...
            {smartOverview.map((item) => (
              <div key={item.label} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${item.tone}`}>{item.value}</span>
                </div>
        ...
        <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 card-static">
            ...
            <ResponsiveContainer width="100%" height={290}>
              <AreaChart data={energyTrendData} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                <defs>
        ...
        {/* ─── Energy & Water Analytics Highlights ─── */}
        <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card-static">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="section-title">Energy Analytics</h2>
                <p className="text-xs text-gray-400 mt-0.5">Usage trends, consumption patterns, and AI-based efficiency insights</p>
              </div>
              <button
                type="button"
          ...
          <div className="card-static">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="section-title">Water Analytics</h2>
                <p className="text-xs text-gray-400 mt-0.5">Water usage tracking, anomaly spotting, and conservation-focused insights</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3">
              {analyticsHighlights.water.map((item) => (
                <div key={item.title} className="rounded-xl border border-gray-100 bg-gray-50 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-gray-800">{item.title}</p>
        ...
        <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 card-static">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: 500, color: '#0f766e', cursor: 'pointer' }}>View All</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {alerts.map((alert) => {
                ...
                return (
                  ...
                  >
                    <div className={`${style.bg}`} style={{ padding: '8px', borderRadius: '10px', flexShrink: 0 }}>
                      <Icon className={`${style.icon}`} style={{ width: '18px', height: '18px' }} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p className={`${style.text}`} style={{ fontSize: '14px', fontWeight: 600, margin: 0, lineHeight: 1.4 }}>{alert.message}</p>
                      <p style={{ fontSize: '12px', color: '#9ca3af', marginTop: '6px' }}>{alert.time}</p>
                    </div>
                  </motion.div>
                )
              })}
          ...
          <div className="lg:col-span-2 card-static">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ padding: '8px', background: '#f0fdfa', borderRadius: '10px' }}>
                  <Lightbulb style={{ width: '18px', height: '18px', color: '#0f766e' }} />
                </div>
              ...
              {recommendations.map((rec) => {
                ...
                return (
                  <motion.div
                    ...
                    className="group hover:bg-primary-50/30"
                  >
                    <div style={{
                      padding: '10px',
                      background: '#ffffff',
                      ...
                      <RecIcon style={{ width: '22px', height: '22px', color: '#0f766e' }} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                        <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#1f2937', margin: 0 }}>{rec.title}</h3>
                        <span className={`${priorityBadge[rec.priority]}`} style={{
                          padding: '3px 10px',
                          fontSize: '11px',
                          fontWeight: 700,
                          borderRadius: '20px',
                      ...
                      <p style={{ fontSize: '13px', color: '#6b7280', lineHeight: 1.6, margin: 0 }}>{rec.description}</p>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '12px' }}>
                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#059669' }}>{rec.impact}</span>
                        <ArrowRight style={{ width: '14px', height: '14px', color: '#9ca3af' }} className="group-hover:text-primary-500 group-hover:translate-x-1 transition-all" />
                      </div>
                    </div>
                  </motion.div>
                )
...
function KpiCard({ icon: Icon, iconBg, iconColor, label, value, unit, change, sparkData }) {
...
function EcoMetric({ label, value, change }) {
  return (
