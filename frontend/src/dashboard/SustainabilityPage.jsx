  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-[1400px] mx-auto space-y-12 pb-10">
      ...
      <motion.div variants={fadeUp} className="card-static" style={{ padding: '36px 40px' }}>
        <h2 className="section-title" style={{ marginBottom: 24 }}>Academic Framework References</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={a.framework} style={{ padding: 16 }} className="bg-gray-50 rounded-xl">
              <p className="text-xs font-semibold text-gray-800">{a.framework}</p>
              <p className="text-[10px] text-gray-500 mt-1">{a.ref}</p>
              <p className="text-[10px] text-primary-600 font-medium mt-1">{a.domain}</p>
            </div>
          ))}
        </div>
      </motion.div>
