...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 px-5 py-4">
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-[1400px] mx-auto space-y-12 pb-10">
      ...
      <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <button style={{
            ...
            background: 'linear-gradient(135deg, #0891b2, #06b6d4)', border: 'none', borderRadius: 12,
            color: '#fff', cursor: 'pointer', boxShadow: '0 2px 8px rgba(6,182,212,0.3)',
        ...
        {[
          ...
          return (
            <div key={kpi.label} className="kpi-card">
              ...
              <div className="mt-4"><span className="text-2xl font-bold text-gray-900">{kpi.value}</span></div>
      ...
      <motion.div variants={fadeUp} className="card-static" style={{ padding: '36px 40px' }}>
        <h2 className="section-title" style={{ marginBottom: 24 }}>Today's Water Profile</h2>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="card-static" style={{ padding: '36px 40px' }}>
          <h2 className="section-title" style={{ marginBottom: 28 }}>Fixture Breakdown</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {fixtureData.map((f) => (
              <div key={f.name} className="flex items-center" style={{ gap: 18 }}>
                <span className="text-sm text-gray-600 flex-shrink-0" style={{ width: 130 }}>{f.name}</span>
                ...
                <span className="text-sm font-semibold text-gray-700 text-right" style={{ width: 50 }}>{f.usage} L</span>
                <span className="text-xs text-gray-400 text-right" style={{ width: 44 }}>{f.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card-static" style={{ padding: '36px 40px' }}>
          <h2 className="section-title" style={{ marginBottom: 28 }}>Weekly Consumption</h2>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={weeklyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} unit=" L" />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey="water" fill="#06b6d4" radius={[6, 6, 0, 0]} barSize={36} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="card-static" style={{ padding: '36px 40px' }}>
        ...
        <div className="flex items-center bg-emerald-50 border border-emerald-200 rounded-2xl" style={{ padding: 28, gap: 20 }}>
          <div className="bg-emerald-100 rounded-xl flex-shrink-0" style={{ padding: 12 }}>
            <Shield className="w-6 h-6 text-emerald-600" />
          </div>
          <div className="flex-1">
            <p className="text-sm font-semibold text-emerald-700">All Systems Normal</p>
            <p className="text-xs text-emerald-600" style={{ marginTop: 6 }}>Last check: 5 minutes ago · Next check in 10 minutes</p>
          </div>
          <button className="px-4 py-2 text-xs font-medium text-emerald-700 bg-white border border-emerald-200 rounded-lg hover:bg-emerald-50 transition-colors">
            Run Test
          </button>
        </div>
      </motion.div>
