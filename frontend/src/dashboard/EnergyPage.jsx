    ...
    <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-[1400px] mx-auto space-y-12 pb-10">
      ...
      <motion.div variants={fadeUp} className="card-static">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">Actual consumption vs AI forecast with anomaly markers</p>
          </div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="card-static" style={{ padding: '36px 40px' }}>
          <h2 className="section-title" style={{ marginBottom: 28 }}>Appliance Breakdown</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {applianceData.map((app) => (
              <div key={app.name} className="flex items-center" style={{ gap: 18 }}>
                <span className="text-sm text-gray-600 flex-shrink-0" style={{ width: 110 }}>{app.name}</span>
                ...
                <span className="text-sm font-semibold text-gray-700 text-right" style={{ width: 80 }}>{app.consumption} kWh</span>
                <span className="text-xs text-gray-400 text-right" style={{ width: 44 }}>{app.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Monthly Trend */}
        <div className="card-static" style={{ padding: '36px 40px' }}>
          <h2 className="section-title" style={{ marginBottom: 28 }}>Monthly Consumption</h2>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={monthlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey="consumption" fill="#14b8a6" radius={[6, 6, 0, 0]} barSize={36} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="card-static" style={{ padding: '36px 40px' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {anomalies.map((a, i) => (
            <div key={i} className="flex items-start bg-amber-50 border border-amber-200 rounded-2xl" style={{ padding: 28, gap: 20 }}>
              <div className="bg-amber-100 rounded-xl flex-shrink-0" style={{ padding: 12 }}>
                <AlertTriangle className="w-5 h-5 text-amber-600" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-3" style={{ marginBottom: 10 }}>
                  <span className="text-sm font-semibold text-gray-800">{a.device}</span>
                  <span className="px-2.5 py-1 text-[10px] font-bold bg-amber-100 text-amber-700 rounded-full">{a.confidence}% confidence</span>
                </div>
                <p className="text-sm text-gray-500 leading-relaxed">
                  At {a.time} — Expected {a.expected} kW, measured {a.actual} kW
                </p>
                <div className="flex gap-3" style={{ marginTop: 16 }}>
                  <button className="px-4 py-2 text-xs font-medium bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                    Acknowledge
                  </button>
                  <button className="px-4 py-2 text-xs font-medium text-primary-600 bg-primary-50 rounded-lg hover:bg-primary-100 transition-colors">
                    View Device
                  </button>
                </div>
              </div>
